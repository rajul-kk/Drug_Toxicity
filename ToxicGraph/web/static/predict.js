...
function renderChart(isTestSet, gt){
  ...
  chartEl.on('plotly_click', async evtData => {
    ...
    const ti = info[pt.x];
    showTaskPopover(pt.x, ti || null, evtData.event);
  });
}

// ── task info (loaded once from static JSON) ───
var _taskInfo = null;
...
function showTaskPopover(taskName, info, evt){
  ...
  const dsCol = info
    ? (info.dataset==='tox21' ? 'var(--green)' : info.dataset==='clintox' ? 'var(--purple)' : 'var(--blue)')
    : 'var(--text-3)';
  const badge = info ? `<span class="ds-pill ${info.dataset}" style="font-size:10px">${info.dataset}</span>` : '';
